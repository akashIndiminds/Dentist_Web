...
import { Star, Quote, ChevronLeft, ChevronRight, CheckCircle, MessageCircle } from 'lucide-react'

export const Testimonials = () => {
  const [currentTestimonial, setCurrentTestimonial] = useState(0)

  const whatsappNumber = "919876543210" // Replace with actual number
  const whatsappMessage = "Hi Doctor, I want to book an appointment after reading your testimonials."
    ...
    return (
      <div className="flex text-yellow-400">
        {[...Array(5)].map((_, i) => (
          <Star
            key={i}
            className={`h-3 w-3 md:h-4 md:w-4 lg:h-5 lg:w-5 ${i < rating ? 'fill-current' : 'stroke-current fill-transparent'}`}
          />
        ))}
      </div>
    )
  ...
  return (
    <section id="testimonials" className="py-8 md:py-12 lg:py-20 bg-gradient-to-br from-blue-50 via-white to-blue-50 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 relative">
        ...
        <div className="text-center mb-8 md:mb-12 lg:mb-16">
          <div className="inline-flex items-center px-3 md:px-4 py-1.5 md:py-2 bg-blue-100 rounded-full mb-3 md:mb-6">
            ...
            <span className="text-xs md:text-sm font-medium text-blue-700">Patient Stories</span>
          </div>
          ...
          <p className="text-sm md:text-base lg:text-lg xl:text-xl text-gray-600 max-w-2xl lg:max-w-3xl mx-auto leading-relaxed px-2">
            Real stories from patients who experienced our <span className="font-semibold text-blue-600">exceptional dental care</span>.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 lg:gap-6 xl:gap-8 mb-8 md:mb-12">
          <div className="text-center bg-white rounded-lg md:rounded-xl lg:rounded-2xl p-3 md:p-4 lg:p-6 shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-[1.02] md:hover:scale-105">
            <div className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-blue-600 mb-1 md:mb-2">4.9</div>
            <div className="text-gray-600 font-medium mb-1 md:mb-2 text-xs md:text-sm">Average Rating</div>
            <div className="flex justify-center">
              {renderStars(5)}
            </div>
          </div>
          <div className="text-center bg-white rounded-lg md:rounded-xl lg:rounded-2xl p-3 md:p-4 lg:p-6 shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-[1.02] md:hover:scale-105">
            <div className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-blue-600 mb-1 md:mb-2">500+</div>
            <div className="text-gray-600 font-medium text-xs md:text-sm">Google Reviews</div>
          </div>
          <div className="text-center bg-white rounded-lg md:rounded-xl lg:rounded-2xl p-3 md:p-4 lg:p-6 shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-[1.02] md:hover:scale-105">
            <div className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-blue-600 mb-1 md:mb-2">98%</div>
            <div className="text-gray-600 font-medium text-xs md:text-sm">Recommend Us</div>
          </div>
          <div className="text-center bg-white rounded-lg md:rounded-xl lg:rounded-2xl p-3 md:p-4 lg:p-6 shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-[1.02] md:hover:scale-105">
            <div className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-blue-600 mb-1 md:mb-2">1000+</div>
            <div className="text-gray-600 font-medium text-xs md:text-sm">Happy Patients</div>
          </div>
        </div>
