...
function GoogleMapComponent() {
  ...
  return (
    <div className="relative">
      {/* Map Container - Fixed Responsive Heights */}
      <div className="h-64 sm:h-72 md:h-80 lg:h-96 bg-gradient-to-br from-blue-100 to-blue-50 relative overflow-hidden rounded-lg md:rounded-xl">
        ...
        {/* Overlay Controls - Fixed Responsive */}
        <div className="absolute top-2 sm:top-3 md:top-4 left-2 sm:left-3 md:left-4 right-2 sm:right-3 md:right-4 z-10">
          <div className="bg-white/95 backdrop-blur-sm rounded-lg md:rounded-xl p-3 md:p-4 shadow-lg border border-white/20">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              {/* Location Info */}
              <div className="flex items-start space-x-2 sm:space-x-3 flex-1">
                <div className="w-8 h-8 md:w-10 md:h-10 bg-gradient-to-r from-blue-600 to-blue-700 rounded-lg flex items-center justify-center flex-shrink-0 shadow-md">
                  <MapPin className="h-4 w-4 md:h-5 md:w-5 text-white" />
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-gray-900 text-sm md:text-base leading-tight truncate">RG&apos;s Smile Zone</h3>
              ...
              <div className="flex space-x-2 flex-shrink-0 w-full sm:w-auto">
                <button
                  ...
                  disabled={isLoading}
                  className="flex-1 sm:flex-none px-2 sm:px-3 md:px-4 py-2 bg-blue-600 text-white rounded-lg text-xs md:text-sm font-medium hover:bg-blue-700 transition-all duration-200 flex items-center justify-center space-x-1 sm:space-x-2 disabled:opacity-50 shadow-md hover:shadow-lg"
                >
                  <Navigation className="h-3 w-3 md:h-4 md:w-4" />
                  <span className="hidden sm:inline">{isLoading ? 'Finding...' : 'My Location'}</span>
                  <span className="sm:hidden">{isLoading ? 'Find' : 'Location'}</span>
                </button>
                
                <button
                  onClick={openInGoogleMaps}
                  className="flex-1 sm:flex-none px-2 sm:px-3 md:px-4 py-2 bg-green-600 text-white rounded-lg text-xs md:text-sm font-medium hover:bg-green-700 transition-all duration-200 flex items-center justify-center space-x-1 sm:space-x-2 shadow-md hover:shadow-lg"
                >
            ...
            {/* Status Messages */}
            {error && (
              <div className="mt-3 p-2 md:p-3 bg-red-50 border border-red-200 rounded-lg">
...
export default function ContactPage() {
  const contactInfo = [
    {
      icon: Phone,
      title: 'Call Us',
      details: '+91 62909 39189',
      subtext: 'Mon-Fri: 9AM-7PM',
      action: 'tel:+916290939189',
      gradient: 'from-blue-500 to-blue-600'
    },
    {
      icon: Mail,
      title: 'Email Us',
      details: 'info@drrajatdental.com',
      subtext: 'Quick response guaranteed',
      action: 'mailto:info@drrajatdental.com',
  ...
  return (
    <div id="contact" className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      <section className="bg-gradient-to-br from-blue-50 via-white to-blue-50 py-12 md:py-16 lg:py-20 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {contactInfo.map((info, index) => {
              ...
              const isExternal = info.title === 'Visit Us'
              return (
                <a
                  key={index}
                  href={info.action}
                  ...
                  className="group bg-white/90 backdrop-blur-sm p-6 md:p-8 rounded-xl lg:rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 border border-white/20 text-center relative overflow-hidden"
                >
      ...
      {/* Office Info Section - Fixed Layout */}
      <section className="py-8 md:py-12 lg:py-16 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="bg-white/90 backdrop-blur-sm p-6 md:p-8 rounded-2xl shadow-xl border border-white/20">
              ...
              <div className="space-y-3 md:space-y-4">
                <div className="flex justify-between items-center py-3 px-4 bg-blue-50 rounded-xl">
                  ...
                  <span className="font-bold text-blue-600 text-sm md:text-base">9AM - 7PM</span>
                </div>
                <div className="flex justify-between items-center py-3 px-4 bg-blue-50 rounded-xl">
                  <span className="text-gray-700 text-sm md:text-base font-medium">Saturday</span>
                  <span className="font-bold text-blue-600 text-sm md:text-base">9AM - 4PM</span>
                </div>
                ...
                <div className="flex justify-between items-center py-3 px-4 bg-gradient-to-r from-emerald-50 to-emerald-100 rounded-xl border border-emerald-200">
                  <span className="text-emerald-700 text-sm md:text-base font-bold">Emergency</span>
                  <span className="font-bold text-emerald-700 text-sm md:text-base">24/7</span>
                </div>
              </div>
            </div>
